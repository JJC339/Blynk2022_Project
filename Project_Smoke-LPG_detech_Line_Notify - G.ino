...
BlynkTimer timer;
WidgetLED alertLED(V5);


#include <TridentTD_LineNotify.h>
#define LINE_TOKEN  ""

int co2,smoke,lpg,limit,alert = 0;
...
#define         RL_VALUE                     (1.5)     //define the load resistance on the board, in kilo ohms
...
void setup()
{
  ...
  timer.setInterval(5000L,updatePpmAndCompare);
...
{
  Serial.println("run UpdatePpmAndCompare function");
  co2 = MQGetGasPercentage(MQRead(MQ_PIN)/Ro,GAS_CO);
  ...
  if(co2 > limit)
  {
    ...
    LINE.notify("ตรวจพบ CO2 เกินกำหนด!!");
    LINE.notify(co2);
  }
  else
  {
